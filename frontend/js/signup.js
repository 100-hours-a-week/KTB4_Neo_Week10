...
if (signupForm) {
  ...
  function getProfileError() {
    if (isProfileUploading) return "* 프로필 사진을 업로드하는 중입니다.";
    if (!profileImageInput.files.length) return "* 프로필 사진을 추가해주세요.";
    if (!profileImageUrl) return "* 프로필 사진 업로드에 실패했습니다. 다시 선택해주세요.";
    return "";
  }
  ...
  profileImageInput.addEventListener("change", async () => {
    ...
    revokeProfilePreviewUrl = setObjectUrlPreview(file, (objectUrl) => {
      profilePreview.style.backgroundImage = `url("${objectUrl}")`;
    });
