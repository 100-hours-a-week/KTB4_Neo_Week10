...
const postUpdatedAt = document.querySelector("#post-updated-at");
...
if (!postId || Number.isNaN(Number(postId))) {
  alert("게시글 정보를 찾을 수 없습니다. 게시글 목록 페이지로 이동합니다.");
  window.location.href = "./post-list.html";
  throw new Error("Invalid postId in query parameters.");
}
...
function countActiveComments(comments) {
  return comments.reduce((total, comment) => {
    const parentCount = comment.deleted ? 0 : 1;
    const replyCount = (comment.replies || []).filter((reply) => !reply.deleted).length;

    return total + parentCount + replyCount;
  }, 0);
}
...
function updateCommentCount() {
  ...
  commentCountElement.textContent = formatCount(countActiveComments(commentsState));
}
...
function updateCommentBodyInState(commentId, commentBody) {
  commentsState = commentsState.map((comment) => {
    ...
    return {
      ...
      replies: (comment.replies || []).map((reply) =>
        String(reply.commentId) === String(commentId)
          ? { ...reply, commentBody }
          : reply
      ),
    };
...
function addReplyToState(parentCommentId, reply) {
  commentsState = commentsState.map((comment) => {
    if (String(comment.commentId) !== String(parentCommentId)) {
      return comment;
    }
...
function updateCommentSubmitState() {
  ...
  commentSubmitButton.disabled = !hasCommentBody || isCommentSubmitting;
  if (isCommentSubmitting) {
    commentSubmitButton.textContent = editingTarget ? "수정 중..." : "등록 중...";
    return;
  }
...
async function fetchReportTypes() {
  const data = await apiRequest("/reports/types", {
...
function createCommentActionButtons(comment, isReply = false) {
  ...
  if (isCurrentUser(comment.userId) && !comment.deleted) {
    ...
    deleteButtonElement.setAttribute("aria-label", isReply ? "대댓글 삭제" : "댓글 삭제");
    deleteButtonElement.title = isReply ? "대댓글 삭제" : "댓글 삭제";
...
async function loadPostDetail() {
  const { post, author, meta } = await apiRequest(`/posts/${postId}`, {
    errorMessage: "게시글을 불러오지 못했습니다.",
  });
  const postImageUrl = post.postImage?.trim();
  ...
  if (postImageUrl && postImageUrl !== "null") {
    ...
    postImage.src = resolveImageUrl(postImageUrl);
  } else {
...
reportForm?.addEventListener("submit", async (event) => {
  ...
  try {
    await apiRequest(`/posts/${postId}/reports`, {
      method: "POST",
      headers: {
...
likeButton?.addEventListener("click", async () => {
  try {
    const likeResult = await apiRequest(`/posts/${postId}/likes`, {
      method: isLiked ? "DELETE" : "POST",
      errorMessage: "좋아요 처리에 실패했습니다.",
    });

    isLiked = likeResult.liked;
    likeCount = likeResult.likes;
    likeButton.classList.toggle("is-active", isLiked);
...
commentForm?.addEventListener("submit", async (event) => {
  ...
  try {
    if (editingTarget) {
      await apiRequest(`/comments/${editingTarget.commentId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      updateCommentBodyInState(editingTarget.commentId, commentBody);
      renderComments();
    } else if (replyingTarget) {
      const reply = await apiRequest(`/comments/${replyingTarget.parentCommentId}/replies`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      addReplyToState(replyingTarget.parentCommentId, reply);
      renderComments();
    } else {
...
commentList?.addEventListener("click", async (event) => {
  ...
  if (commentDeleteButton || replyDeleteButton) {
    ...
    try {
      await apiRequest(`/comments/${targetItem.dataset.commentId}`, {
        method: "DELETE",
